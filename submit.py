...
################################
    ...
    y_new=np.where(y>0,1,-1) #map 1->1 and 0->-1
...
################################
    ...
    X_new=np.flip(X_new,axis=1)
    
    X_new=np.c_[X_new,np.ones(X_new.shape[0])]# Adding extra column of ones to account for bias
    
    X_new=np.transpose(khatri_rao(khatri_rao(np.transpose(X_new),np.transpose(X_new)),np.transpose(X_new)))
    return X_new
...
################################
    ...
    W_extended=np.random.normal(0, 1, X_new.shape[1])
    batchsize=16
    stepsize=0.0001
    epochs=1500
    j=0#for iterating batch no
    C=10
    i=0#for iterating epochs

    
    no_of_batches=(len(X)+batchsize-1)//batchsize
...
################################
        ...
        if(i<epochs): #dont update weights after num_epochs
           X_1=X_new[j*batchsize:min(batchsize*(j+1),X.shape[0])]
           y_1=y[j*batchsize:min(batchsize*(j+1),y.shape[0])]
           cache=np.multiply(X_1,y_1[:,np.newaxis]) #computed once and reused
           g=np.dot(cache,W_extended)
           g=np.where( g < 1, -1, 0 )
           gradient=W_extended+C*np.dot(g,cache)
           W_extended=W_extended-stepsize*gradient
           W=W_extended
